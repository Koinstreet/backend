...
class Transport:
    ...
    def send(self, message):
        asyncio.Task(self.invoke_queue.put(InvokeEvent(message)), loop=self._connection.mainloop)

    def close(self):
        asyncio.Task(self.invoke_queue.put(CloseEvent()), loop=self._connection.mainloop)

    # -----------------------------------
    # Private Methods

    def _set_loop_and_queue(self):
        asyncio.set_event_loop(self._connection.mainloop)
        self.invoke_queue = asyncio.Queue(loop=self._connection.mainloop)

    def _connect(self):
        #self._conn_handler = asyncio.ensure_future(self._socket(self.ws_loop), loop=self.ws_loop)
        self._conn_handler = asyncio.ensure_future(self._socket(), loop=self._connection.mainloop)
        
    async def _socket(self):
        async with websockets.connect(self._ws_params.socket_url, extra_headers=self._ws_params.headers,
                                      loop=self._connection.mainloop) as self.ws:
            self._connection.started = True
            await self._master_handler(self.ws)

    async def _master_handler(self, ws):
        consumer_task = asyncio.ensure_future(self._consumer_handler(ws), loop=self._connection.mainloop)
        producer_task = asyncio.ensure_future(self._producer_handler(ws), loop=self._connection.mainloop)
        done, pending = await asyncio.wait([consumer_task, producer_task],
                                           loop=self._connection.mainloop, return_when=asyncio.FIRST_EXCEPTION)
